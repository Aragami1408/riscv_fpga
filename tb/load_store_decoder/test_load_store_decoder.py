...
async def ls_unit_test(dut):
    ...
    for _ in range(10):
        ...
        for offset in range(4):
            dut.alu_result_address.value = word | offset
            await Timer(1, unit="ns")
            print(f"reg_read: {hex(dut.reg_read.value)}")
            print(f"address: {hex(dut.alu_result_address.value)}")
            print(f"\tbyte_enable: {bin(dut.byte_enable.value)}")
            print(f"\tdata: {hex(dut.data.value)}\n")
            if offset == 0b00:
